...
void main() {
  ...
  List<Inventory> inventoryCache = [];

  // utility to query blogs and return length
  group('GraphQL with model using custom primary key', () {
    Future<bool> deleteInventory(ModelIdentifier modelIdentifier) async {
      final req = ModelMutations.deleteByModelIdentifier<Inventory>(
          Inventory.classType, modelIdentifier);
      await Amplify.API.mutate(request: req).response;
      return true;
    }

    Future<Inventory> addInventory(Inventory inventory) async {
      final request = ModelMutations.create(inventory);
      final response = await Amplify.API.mutate(request: request).response;

      Inventory? data = response.data;

      if (data == null) {
        throw Exception(
            'Null response while creating post. Response errors: ${response.errors}');
      }
    ...
    Future<Inventory> updateInventory(Inventory inventory) async {
      final request = ModelMutations.update(inventory);
      final response = await Amplify.API.mutate(request: request).response;
    ...
    Future<void> deleteAllInventories() async {
      inventoryCache.forEach((inventory) async {
        final request = ModelMutations.deleteByModelIdentifier(
          Inventory.classType,
        ...
        inventoryCache.removeWhere(
            (inventory) => inventory.modelIdentifier == data.modelIdentifier);
      });
    }
    ...
    group('queries', () {
      testWidgets('should fetch', (WidgetTester tester) async {
        const listInventories = 'listInventories';
        const items = 'items';
        ...
        expect(
            data[listInventories][items], hasLength(greaterThanOrEqualTo(0)));
      });
      ...
      testWidgets('should GET a blog with Model helper',
        ...
        Inventory addedInventory = await addInventory(inventory);

        final req = ModelQueries.get(Inventory);
