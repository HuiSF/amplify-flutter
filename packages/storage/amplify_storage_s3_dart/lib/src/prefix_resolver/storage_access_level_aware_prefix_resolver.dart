...
class StorageAccessLevelAwarePrefixResolver implements StorageS3PrefixResolver {
  ...
  StorageAccessLevelAwarePrefixResolver({
    required TokenAmplifyAuthProvider identityProvider,
    String? delimiter,
  })  : _delimiter = delimiter ?? '/',
        _identityProvider = identityProvider;

  final String _delimiter;
  final TokenAmplifyAuthProvider _identityProvider;
  ...
  Future<String> resolvePrefix({
    ...
    final identityId = await _identityProvider.getIdentityId();

    if (storageAccessLevel == StorageAccessLevel.guest) {
      return '${storageAccessLevel.defaultPrefix}$_delimiter';
    }

    return '${storageAccessLevel.defaultPrefix}$_delimiter$identityId$_delimiter';
  }
}
