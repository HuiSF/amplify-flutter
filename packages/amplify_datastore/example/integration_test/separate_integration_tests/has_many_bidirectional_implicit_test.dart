  ...
  group('HasMany (bi-directional with implicit connection field', () {
      ...
      if (enableCloudSync) {
        var eventGetter = getExpectedSubscriptionDataProcessedEvent(
          eventMatcher: (event) {
            ...
            if (model is HasManyParentBiDirectionalImplicit) {
      ...
      for (var child in children) {
        if (enableCloudSync) {
          var eventGetter = getExpectedSubscriptionDataProcessedEvent(
            eventMatcher: (event) {
              var model = event.element.model;
              if (model is HasManyChildBiDirectionalImplicit) {
                return model.id == child.id;
              }
    ...
    testWidgets('delete parent (cascade delete associated children)',
      ...
      if (enableCloudSync) {
        ...
        parentChildrenEvents.addAll(childrenEventsGetter);
