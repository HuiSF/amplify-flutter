            ...
            if (serializedMap.containsKey("queryPredicateOperation")) {
                ...
                when (queryFieldOperatorMap["operatorName"]) {
                    "equal" -> return when (operand) {
                        is List<*> -> {
                            val identifierFieldsValues = operand.cast<Map<String, Serializable>>().map { it.values.first() }
                            val identifier = ModelIdentifier.Helper.getIdentifier(identifierFieldsValues[0],
